...
// 🚀 Built-in Working ProxyIP Pool (CMLiu-Free)
...
var proxyIP = proxyIPPool[0];

// 🔗 Optional: GitHub Raw Link for Dynamic IP List
// Leave empty or with YOUR_USERNAME to disable
var githubProxyURL = "";
...
// 🎲 Random ProxyIP Selector from Built-in Pool
function getRandomProxyIP() {
    if (!proxyIPPool || proxyIPPool.length === 0) return proxyIP;
    return proxyIPPool[Math.floor(Math.random() * proxyIPPool.length)];
}

// GitHub မှ Proxy IP များ ဆွဲယူပေးမည့် Function
async function getDynamicProxyIP(defaultProxy, rawUrl) {
    if (!rawUrl || rawUrl.includes("YOUR_USERNAME") || rawUrl.trim() === "") {
        return defaultProxy;
    }
    try {
        const response = await fetch(rawUrl, {
            cf: { cacheTtl: 300, cacheEverything: true } // 5 မိနစ် Cache
        });
        if (response.ok) {
...
// 🧹 IPv6 Brackets Stripper for connect()
function cleanHostname(hostname) {
    if (!hostname) return hostname;
    // Remove [ and ] brackets if present (IPv6 format)
...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, responseHeader, log) {
    async function connectAndWrite(address, port) {
        const cleanAddr = cleanHostname(address);
        const tcpSocket2 = connect({ hostname: cleanAddr, port });
        remoteSocket.value = tcpSocket2;
        log(`Connected to ${cleanAddr}:${port}`);
    ...
    async function retry() {
        // 🎲 Try built-in pool first, then GitHub if configured
        let activeProxy = await getDynamicProxyIP(getRandomProxyIP(), githubProxyURL);

        // If GitHub failed or disabled, use random from pool
        if (!activeProxy || activeProxy === proxyIP) {
            activeProxy = getRandomProxyIP();
        }
